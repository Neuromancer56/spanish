...
for key,value in pairs(phonics) do
	minetest.register_node("spanish:"..key, {
	description = key,
...
--		minetest.chat_send_all("phonic.on_punch:" .. pos.x ..">" )
	...
	minetest.register_node("spanish:"..key.."_active", {
	...
	if  phonics[key].is_silent ==1 then	
		minetest.register_node("spanish:0"..key, {
...
minetest.register_node("spanish:question_mark", {
...
minetest.register_node("spanish:exclamation_point", {
...
minetest.register_node("spanish:period", {
	description = "Period",
	tiles = {
	"period.png",},
	is_ground_content = true,
...
minetest.register_node("spanish:SayWord", {
...
minetest.register_node("spanish:PaperStart", {
	...
	after_dig_node = function(pos, oldnode, oldmetadata, digger)
        ...
        local itemstack = ItemStack("spanish:PaperStart")
...
function clear_page(pos)
	...
	replace_page(clear_pos, "x", 1, "spanish:BlankPaper")
	replace_page(clear_pos, "x", -1, "spanish:BlankPaper")
	replace_page(clear_pos, "z", 1, "spanish:BlankPaper")
	replace_page(clear_pos, "z", -1, "spanish:BlankPaper")
	replace_column(clear_pos, "spanish:BlankPaper")
	page_start = pos
	page_end = pos
end
	    
minetest.register_node("spanish:PaperFinish", {
...
minetest.register_node("spanish:BlankPaper", {
...
function revertnode(parms)  
 	local pos3 = parms[1]
 	local node3= parms[2]
 	minetest.env:remove_node(pos3,{name="spanish:"..node3.."_active"})
 	minetest.env:place_node(pos3,{name="spanish:"..node3})
end
...
function activate_node(anparms)
	...
	local duration = anparms[3]
 	minetest.env:remove_node(pos2,{name="spanish:"..nodename})
 	minetest.env:place_node(pos2,{name="spanish:"..nodename.."_active"}) 
...
--see if there are any sounds next to mouth on this axis.  (or if there is a BlankPage before it.) If not, do nothing.  
	...
	if b4_nodename ~="spanish:BlankPaper" then	
	   	if test_nodename_prefix ~="spanish" or test_nodename_suffix == "BlankPaper" or test_nodename_suffix =="PaperStart" then 
	   		return
	    end
	end
    ...
    repeat  --go to next row 1 time    	      
		repeat  --continue sounding on this row until you reach blank paper or a non phonics node
			...
			if  delay ~=nil and nodename_prefix =="spanish"  then 		
		...
		until nodename_prefix ~="spanish" or nodename_suffix == "BlankPaper"
--if I will find a BlankPaper in this row, then dont look in next row
		...
		until find_blank_nodename_prefix ~="spanish"--prefix not phonics
...
--we're not done, so search next row for more phonics to sound out
		...
		until new_row_nodename_prefix ~="spanish"
...
function write_message_to_page_on_axis(pos, axis, direction,message)
	...
	if firstname_prefix == "spanish" then  --we have phonics on this axis		      
		repeat
		...
		until nodename_prefix ~="spanish" or message_index > string.len(message)		
	end
end
...
function write_page_row(pos, axis, direction, message)
	...
	if nodename=="spanish:PaperStart" then
		if axis == "x" then
			replace_pointer.x=replace_pointer.x+direction
			new_nodename ="spanish:SayWord" 
...
--5repeat 1-4 until node prefix at current location is not phonics	
	repeat
		...
		local nodename_prefix = nodenamearray[1]
		local nodename_suffix = nodenamearray[2]
		if nodename_prefix =="spanish"  then 			
			if new_nodename =="spanish:BlankPaper" then
...
--and have some code behind the scenes automatically bump the message_index	
	...
	if message_index <= string.len(message) then
		...
		if current_char_in_message==" " then 
			current_phonic_in_message = "spanish:BlankPaper"
		elseif current_char_in_message=="." then 
			current_phonic_in_message = "spanish:period"
		elseif current_char_in_message=="?" then 
			current_phonic_in_message = "spanish:question_mark"			
		elseif current_char_in_message=="!" then 
			current_phonic_in_message = "spanish:exclamation_point"						
		else
			...
			current_phonic_in_message = "spanish:"..phonic_builder
		end
		message_index = message_index +1	
	else
		current_phonic_in_message = "spanish:BlankPaper"
...
function replace_column(pos, new_nodename)
	...
	repeat
		...
		if nodename_prefix =="spanish" then	
			...
			if new_nodename =="spanish:BlankPaper" and current_nodename ~="spanish:PaperStart" then					
				minetest.env:add_node(replace_column_pos, {name=new_nodename})			
			end
		end
	...
	if page_start ~= nil and math.abs(page_start.x-page_end.x) <20 and math.abs(page_start.y-page_end.y) <20 and math.abs(page_start.z-page_end.z) <20 then 
		if page_end.y <= page_start.y then  --if the page end block was placed at or below page start block
			if page_end.x == page_start.x then
				buildwall(page_start.z, pos.z, page_start.y, pos.y, "x", pos.x, "spanish:BlankPaper")	
			end
			if page_end.z == page_start.z then  
				buildwall(page_start.x, pos.x, page_start.y, pos.y, "z", pos.z, "spanish:BlankPaper")	
			end	
		end	
	end 
end
...
function buildrow(hstart, hend, y, haxis, haxis_fixed, nodename)
	...
	repeat
		...
		if minetest.env:get_node(current_pos).name ~= "spanish:PaperStart" then   --leave paperstart node there.
